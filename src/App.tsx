...
const ProjectCard = ({ project, index }: any) => {
  ...
  return (
    <motion.div 
      ...
      className="group relative overflow-hidden bg-zinc-900/40 border border-white/10 flex flex-col rounded-[2rem] backdrop-blur-sm hover:border-cyan-400/50 transition-all duration-500 min-w-[300px] md:min-w-[400px] snap-start"
    >
      <div className="p-3">
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div ref={containerRef} className="bg-black text-white selection:bg-cyan-500 selection:text-black relative">
      ...
      <nav className="fixed left-6 md:left-10 top-1/2 -translate-y-1/2 z-[100] flex flex-col gap-4">
        {[
          ...
          <div key={item.id} className="group relative flex items-center">
            <Link 
              ...
              className="w-10 h-10 md:w-12 md:h-12 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl flex items-center justify-center hover:bg-cyan-400 hover:text-black transition-all duration-300 shadow-xl"
            >
              <item.icon className="w-4 h-4 md:w-5 md:h-5" />
            </Link>
            
            {/* Hover Label */}
            <div className="absolute left-full ml-4 px-3 py-1 bg-white text-black text-[10px] font-bold tracking-widest rounded opacity-0 -translate-x-2 pointer-events-none group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 whitespace-nowrap">
              {item.label}
            </div>
          </div>
        ))}
  ...
  return (
    <>
      ...
      <section id="projects" className="py-40 border-t border-white/5 relative">
        <div className="px-8 md:px-32 flex justify-between items-end mb-12">
